...
- 卡片：查看目前要複習的單字卡。
- AI 文章：讓 AI 產生日文短文，並自動從文章擷取單字卡。
- 考題：讓 AI 根據已儲存的學習卡出選擇題。
- 資料庫：搜尋與檢視來源、卡片、文章、考題等資料。
- 設定：設定顯示頻率、AI Provider、資料來源與 log。
...
「AI 文章」分頁可以讓 AI 產生日文短文，再自動擷取單字卡。
...
```text
iCloud Drive/Documents/JapaneseLearningCard/store.sqlite
```

如果 iCloud 不可用，會使用本機資料庫：
...
4. 到「考題」產生 AI 選擇題並作答。
5. 到「歷史」查看複習卡片與考試紀錄。
6. 若 AI 呼叫異常，到「設定」打開 AI Log 檢查原因。
