...
private struct RubyFlowLayout: Layout {
    ...
    func sizeThatFits(proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) -> CGSize {
        rows(for: subviews, maxWidth: proposal.width ?? .infinity).size
    }

    func placeSubviews(in bounds: CGRect, proposal: ProposedViewSize, subviews: Subviews, cache: inout ()) {
        let result = rows(for: subviews, maxWidth: bounds.width)
        var y = bounds.minY
    ...
    private func rows(for subviews: Subviews, maxWidth: CGFloat) -> RubyFlowRows {
